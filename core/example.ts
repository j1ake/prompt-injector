...
import type { ConversationContext, Turn, AttackIntent, ObfuscationLevel, Timing } from './src/types.js';

class CustomTestPrimitive extends BasePrimitive {
  readonly id = 'custom-test';
  readonly name = 'Custom Test Primitive';
  readonly description = 'A custom primitive for demonstration';
  readonly timing: Timing[] = ['any'];
  readonly researchNotes = 'Custom implementation for testing';
  readonly obfuscationLevel: ObfuscationLevel = 'none';
  readonly targetIntents: AttackIntent[] = ['safety-bypass'];
...
const customConversation = injector.generateConversation(
  "test custom primitive", 
  { preferredPrimitives: ['custom-test'], maxTurns: 1 }
);
